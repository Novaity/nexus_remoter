...
export default function App() {
  ...
  return (
    <div className="min-h-screen bg-[#020617] text-slate-100 p-6 font-sans select-none overflow-x-hidden">
        ...
        <div>
          ...
          <div className="flex items-center gap-2 mt-2">
            <div className={`w-2.5 h-2.5 rounded-full shadow-lg transition-all duration-500 ${
              state.connectionStatus === 'online' ? 'bg-green-500 shadow-green-500/50' : 'bg-red-500 shadow-red-500/50'
            }`} />
            <input 
      ...
      {editingBtn && (
        <div className="fixed inset-0 z-50 bg-slate-950/90 backdrop-blur-xl flex items-end sm:items-center justify-center p-4 animate-in fade-in">
          <div className="bg-slate-900 w-full max-w-md rounded-[3rem] p-8 border border-white/5 shadow-2xl overflow-y-auto max-h-[90vh]">
            <div className="flex justify-between items-center mb-8">
              <h2 className="text-xl font-black italic tracking-tighter uppercase">DÜZENLE</h2>
              <button onClick={() => setEditingBtn(null)} className="p-2 bg-slate-800 rounded-full"><X size={20} /></button>
            </div>

            <div className="space-y-6">
              <div className="space-y-2">
                <label className="text-[10px] font-bold text-slate-500 uppercase tracking-widest ml-2">İsim & İkon</label>
                <div className="grid grid-cols-2 gap-4">
                  ...
                  >
                    ...
                    <option value="GAME">Sistem / Güç</option>
                  </select>
                </div>
              </div>

              <div className="bg-cyan-500/5 border border-cyan-500/20 p-5 rounded-3xl relative overflow-hidden">
                <div className="flex items-center gap-2 mb-3">
                  <Sparkles size={14} className="text-cyan-400" />
                  <span className="text-[10px] font-black text-cyan-400 uppercase tracking-widest">AI Makro</span>
                </div>
                <textarea 
              ...
              <div className="flex gap-4 pt-4 border-t border-slate-800">
                <button 
                  ...
                  className="flex-1 bg-gradient-to-r from-cyan-500 to-blue-600 text-slate-950 font-black py-4 rounded-2xl shadow-lg flex items-center justify-center gap-2"
                >
                  <Save size={18} /> KAYDET
                </button>
                <button 
