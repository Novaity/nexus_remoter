...
import { 
  ...
  RefreshCw, Power, Terminal, Layers, ChevronRight
} from 'lucide-react';
...
export default function App() {
  ...
  // Otomatik Kaydet ve Bağlantı Kontrolü
  useEffect(() => {
  ...
  const checkConnection = async () => {
    ...
    setState(s => ({ ...s, connectionStatus: 'checking' }));
  ...
  const handleButtonClick = async (btn: ControlButton) => {
    ...
    try {
      ...
      if (!result.success) {
        setLastError(result.error || "İşlem başarısız.");
      }
    } finally {
  ...
  const handleAiGenerate = async () => {
    ...
    try {
      ...
      if (steps && steps.length > 0) {
        ...
        setLastError("AI bu komutu anlayamadı.");
      }
    } catch (e) {
  ...
  return (
    <div className="min-h-screen bg-[#020617] text-slate-100 p-6 pb-24 font-sans select-none">
      {/* Header */}
      <div className="flex justify-between items-center mb-10">
        <div>
          ...
          <div className="flex items-center gap-2 mt-2">
            <div className={`w-2 h-2 rounded-full shadow-lg transition-all duration-500 ${
            ...
            <input 
              className="bg-slate-900/50 border border-slate-800 rounded-lg px-2 py-1 text-[10px] font-mono outline-none focus:border-cyan-500 w-32"
              placeholder="PC IP: 192.168.x.x"
        ...
        <button 
          ...
          className={`p-4 rounded-2xl transition-all ${state.isEditMode ? 'bg-cyan-500 text-slate-950 scale-105 shadow-xl shadow-cyan-500/20' : 'bg-slate-900 border border-slate-800'}`}
        >
          <Settings size={20} />
        </button>
      </div>
      ...
      {lastError && (
        <div className="mb-6 p-4 bg-red-500/10 border border-red-500/20 rounded-2xl flex items-start gap-3 animate-in fade-in zoom-in duration-300">
          ...
          <div className="flex-1">
            <p className="text-[10px] font-bold text-red-500 uppercase tracking-widest">Hata</p>
            <p className="text-xs text-slate-300 mt-1">{lastError}</p>
          </div>
      ...
      <div className="grid grid-cols-2 gap-5">
        ...
        {state.isEditMode && (
          <button 
            onClick={() => {
              const newBtn: ControlButton = { id: Date.now().toString(), label: 'Yeni', color: 'bg-slate-800', icon: 'cpu', steps: [] };
        ...
        <div className="fixed inset-0 z-50 bg-slate-950/90 backdrop-blur-xl flex items-end sm:items-center justify-center p-4 animate-in fade-in duration-300">
          <div className="bg-slate-900 w-full max-w-md rounded-[3rem] p-8 border border-white/5 shadow-2xl overflow-y-auto max-h-[90vh]">
            <div className="flex justify-between items-center mb-8">
              <h2 className="text-xl font-black tracking-tighter italic">DÜZENLE</h2>
            ...
            <div className="space-y-6">
              <div className="space-y-2">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <input 
                    className="bg-slate-800 border border-slate-700 rounded-2xl p-4 outline-none focus:ring-2 ring-cyan-500 transition-all"
                    value={editingBtn.label}
                    placeholder="Buton Adı"
                    onChange={e => setEditingBtn({...editingBtn, label: e.target.value})}
                  />
                  <select 
                    className="bg-slate-800 border border-slate-700 rounded-2xl p-4 outline-none appearance-none"
                    value={editingBtn.icon}
                    onChange={e => setEditingBtn({...editingBtn, icon: e.target.value})}
                  >
                    <option value="cpu">Varsayılan</option>
                    <option value="globe">Web</option>
                    <option value="game">Oyun</option>
                    <option value="power">Sistem</option>
                    <option value="terminal">CMD</option>
                  </select>
                </div>
              </div>
              ...
              <div className="bg-cyan-500/5 border border-cyan-500/20 p-5 rounded-3xl relative overflow-hidden group">
                ...
                <button 
                  ...
                  className="w-full bg-cyan-500 text-slate-950 font-black py-3 rounded-xl text-[10px] uppercase tracking-widest hover:bg-cyan-400 active:scale-95 transition-all"
                >
                  Adımları Oluştur
                </button>
              </div>
              ...
              <div className="space-y-3">
                <div className="flex justify-between items-center ml-2">
                  <label className="text-[10px] font-bold text-slate-500 uppercase tracking-widest">Akış ({editingBtn.steps.length})</label>
                </div>
                <div className="space-y-2 max-h-48 overflow-y-auto pr-2 custom-scrollbar">
                  {editingBtn.steps.map((step, i) => (
                    <div key={i} className="flex items-center gap-3 bg-slate-800/50 p-3 rounded-2xl border border-white/5">
                      <div className="bg-cyan-500/20 p-2 rounded-lg text-cyan-400 text-[8px] font-bold">{i+1}</div>
                      <div className="flex-1 min-w-0">
                        <p className="text-[10px] font-bold truncate uppercase tracking-tighter">{step.type}</p>
                  ...
                  {editingBtn.steps.length === 0 && (
                    <div className="text-center py-6 border-2 border-dashed border-slate-800 rounded-3xl text-slate-600 text-xs italic">
              ...
              <div className="flex gap-4 pt-4">
                <button 
                  ...
                  className="flex-1 bg-gradient-to-r from-cyan-500 to-blue-600 text-slate-950 font-black py-4 rounded-2xl shadow-lg shadow-cyan-500/20 active:scale-95 transition-all flex items-center justify-center gap-2"
                >
                  <Save size={18} /> KAYDET
                </button>
                <button 
      ...
      {/* Global Status Bar */}
      {state.isExecuting && (
        <div className="fixed bottom-10 left-1/2 -translate-x-1/2 z-50 flex items-center gap-4 bg-cyan-500 text-slate-950 px-8 py-4 rounded-full font-black text-xs shadow-2xl animate-bounce">
          <RefreshCw className="animate-spin" size={16} />
          KOMUTLAR ÇALIŞTIRILIYOR
        </div>
      )}
    </div>
  );
